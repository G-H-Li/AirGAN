...
class AssignmentMaker:
    def __init__(self, config: Config):
        self.distance_list = [50, 200]
        self.dist_list_len = len(self.distance_list)
        self.angle_list = [0, 45, 90, 135, 180, 225, 270, 315]
        self.angle_list_len = len(self.angle_list)
        self.projection_len = len(self.distance_list) * len(self.angle_list) + 1

        self.loc_filepath = os.path.join(config.dataset_dir, 'KnowAir_loc_filled.npy')
        self.locations = self.__read_data()
        self.city_num = self.locations.shape[0]
        self.distance_matrix = self.__cal_distance_direction()
        self.assignment = np.zeros((self.city_num, self.projection_len, self.city_num))
        self.__generate_assignment()
        self.__cal_assignment_mean()
        self.assignment = self.assignment.transpose(0, 2, 1)
        np.save(os.path.join(config.dataset_dir, 'KnowAir_assignment.npy'), self.assignment)

    def __read_data(self):
        return np.load(self.loc_filepath)

    def get_dis_idx(self, dis):
        return np.searchsorted(self.distance_list, dis, side='right')

    def get_angle_idx(self, angle):
        return np.searchsorted(self.angle_list, angle, side='right')

    def __cal_distance_direction(self):
        indices = np.arange(self.city_num)
        pairs = np.array(np.meshgrid(indices, indices)).T.reshape(-1, 2)
        distances = haversine(self.locations[pairs[:, 0], 0], self.locations[pairs[:, 0], 1],
                              self.locations[pairs[:, 1], 0], self.locations[pairs[:, 1], 1])
        initial_bearings = initial_bearing(self.locations[pairs[:, 0], 0], self.locations[pairs[:, 0], 1],
                                           self.locations[pairs[:, 1], 0], self.locations[pairs[:, 1], 1])
        # 将结果重塑为矩阵
        distance_matrix = distances.reshape(self.city_num, self.city_num, 1)
        initial_bearing_matrix = initial_bearings.reshape(self.city_num, self.city_num, 1)

        distance_angle_matrix = np.concatenate((distance_matrix, initial_bearing_matrix), axis=-1)
        return distance_angle_matrix

    def __generate_assignment(self):
        for i in range(self.city_num):
            dis_angle = self.distance_matrix[i]
            for j in range(self.city_num):
                if dis_angle[j, 0] == 0:
                    self.assignment[i, 0, j] = 1
                    continue
                dis_idx = self.get_dis_idx(dis_angle[j, 0])
                angle_idx = self.get_angle_idx(dis_angle[j, 1])
                if dis_idx >= self.dist_list_len:
                    continue
                self.assignment[i, dis_idx * self.angle_list_len + angle_idx, j] = 1

    def __cal_assignment_mean(self):
        for i in range(self.city_num):
            for j in range(self.projection_len):
                assignment_idx = self.assignment[i, j, :] != 0
                count = np.sum(assignment_idx)
                if count == 0:
                    continue
                else:
                    self.assignment[i, j, assignment_idx] = 1 / count


if __name__ == '__main__':
    config = Config()
    maker = AssignmentMaker(config)

"""
Created on generate AirFormer mask.npy
https://github.com/microsoft/Cream/blob/main/iRPE/HOW_TO_EQUIP_iRPE.md
"""
