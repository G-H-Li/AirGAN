...
class SimST(nn.Module):
    ...
    def forward(self, pm25_hist, features, city_locs, date_emb):
        ...
        features = features.reshape(self.batch_size, self.hist_len+self.pred_len, -1)
        xn = pm25_hist.view(self.batch_size, -1)
        ...
        for i in range(self.pred_len):
            ...
            graph_emb = self.graph_mlp(features[:, self.hist_len+i].squeeze())
            ...
            temp_hidden = torch.cat(temp_hidden, dim=-1).squeeze()
